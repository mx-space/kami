import {
  faCheckCircle,
  faExclamationCircle,
  faInfoCircle,
  faTimesCircle,
} from '@fortawesome/free-solid-svg-icons'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { FC, useEffect, useRef, useState } from 'react'
import styles from './index.module.css'

const LoadingIcon = () => {
  return (
    <div className={styles['rotate']}>
      <svg viewBox="0 0 512 512" width="1em" height="1em">
        <path
          fill="var(--blue)"
          d="M460.115 373.846l-6.941-4.008c-5.546-3.202-7.564-10.177-4.661-15.886 32.971-64.838 31.167-142.731-5.415-205.954-36.504-63.356-103.118-103.876-175.8-107.701C260.952 39.963 256 34.676 256 28.321v-8.012c0-6.904 5.808-12.337 12.703-11.982 83.552 4.306 160.157 50.861 202.106 123.67 42.069 72.703 44.083 162.322 6.034 236.838-3.14 6.149-10.75 8.462-16.728 5.011z"
        ></path>
      </svg>
    </div>
  )
}

interface MessageProps {
  type: 'success' | 'warn' | 'error' | 'info' | 'loading'
  message: string
  duration?: number
}

const Icon = {
  success: <FontAwesomeIcon icon={faCheckCircle} color={'var(--green)'} />,
  error: <FontAwesomeIcon icon={faTimesCircle} color={'var(--red)'} />,
  info: <FontAwesomeIcon icon={faInfoCircle} color={'var(--blue)'} />,
  warn: <FontAwesomeIcon icon={faExclamationCircle} color={'var(--yellow)'} />,
  loading: <LoadingIcon />,
}
export const Message: FC<MessageProps> = (props) => {
  const { type, message, duration } = props
  const [isAppeared, setAppear] = useState(true)
  const wrapRef = useRef<HTMLDivElement>(null)
  useEffect(() => {
    const timer = setTimeout(() => {
      if (!wrapRef.current) {
        return
      }
      wrapRef.current.classList.add(styles['disappear'])

      wrapRef.current.onanimationend = () => {
        wrapRef.current?.remove()
      }
    }, duration || 2500)
    return () => {
      clearTimeout(timer)
    }
  }, [wrapRef.current])

  return isAppeared ? (
    <div className={styles['wrap']}>
      <div className={styles['inner-wrap']} ref={wrapRef}>
        <div className={styles['icon']}>{Icon[type]}</div>
        <div className={styles['message']}>
          <span>{message}</span>
        </div>
      </div>
    </div>
  ) : null
}

export const MessageContainerPrefixId = `mx-message-container`
export const MessageContainer = () => {
  return (
    <div className={styles['container']} id={MessageContainerPrefixId}></div>
  )
}
